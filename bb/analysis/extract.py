...
class Extractor:
    ...
    async def extract(self, updates: list, context_updates: list | None = None,
        ...
        system = (
            ...
            "- EXCEPTION — PUBLISHED ALLIANCE REPORT: when an update is a site's "
            "alliance write-up that LISTS groups and their members (e.g. 'The "
            "Toolshed: Devens, Dee, Angela, Kamu, Chuk, Haley, Drew and Barrett' "
            "or a bulleted roster list), record EVERY listed group as an alliance "
            "with that exact name and its FULL member list, confidence 0.9. Such a "
            "report states rosters deliberately, so the two rules above do not "
            "apply to it: no in-text agreement is needed and the published name "
            "counts. Also record unnamed pairs it states as deals ('Dee and Devens "
            "are the most solid duo') with no name. Hedges like 'not a serious "
            "thing' lower confidence but still count.\n"
            ...
            "- RELATIONSHIP CHANGES record PERSONAL dynamics between two people, "
            "and only from events that actually occurred between them:\n"
            "  * 'conflict' = a real argument, fight, or visible personal friction "
            "BETWEEN the two. Strategic talk is NOT conflict: targeting, "
            "nominating, 'wants X out', suspicion, or trash-talking someone who "
            "isn't present are game moves, not personal feuds — omit them.\n"
            "  * 'allied' = a genuine personal bond shown in the text (heart-to-"
            "heart, loyalty affirmed to each other) — not two people merely "
            "scheming, which belongs in alliances.\n"
            "  * 'betrayal' = one ACTUALLY betrayed the other (flipped a vote, "
            "exposed a secret, broke a deal — it happened). Planning or fearing a "
            "betrayal is not one.\n"
            "  * 'showmance_start' = genuine mutual romance. Flirty joking, fake "
            "or strategic romance, or one-sided pursuit is NOT a showmance.\n"
